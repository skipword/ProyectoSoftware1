...
// Asegurarse de que las variables solo se declaren una vez
...
function showModalCompra() {
    if (modalOverlayCompra) {
        modalOverlayCompra.style.display = 'flex'; // Muestra el modal de compra
    }
}
...
function closeModalCompra() {
    if (modalOverlayCompra) {
        modalOverlayCompra.style.display = 'none'; // Oculta el modal de compra
    }
}

// Asignar el evento para mostrar el modal de compra solo si el botón existe
const btnComprarCombo = document.getElementById('btnComprarCombo');
if (btnComprarCombo) {
    btnComprarCombo.addEventListener('click', showModalCompra);
}

// Asignar los eventos para confirmar o cancelar la compra solo si los botones existen
if (confirmarCompraButton) {
    confirmarCompraButton.addEventListener('click', function () {
        alert('¡Compra confirmada!');
        closeModalCompra(); // Cerrar el modal después de confirmar
    });
}

if (cancelarCompraButton) {
    cancelarCompraButton.addEventListener('click', closeModalCompra);
}

// Cerrar el modal al hacer clic fuera del contenido
if (modalOverlayCompra) {
    modalOverlayCompra.addEventListener('click', function (event) {
        if (event.target === modalOverlayCompra) {
            closeModalCompra(); // Cierra si el clic fue fuera del contenido
        }
    });
}
